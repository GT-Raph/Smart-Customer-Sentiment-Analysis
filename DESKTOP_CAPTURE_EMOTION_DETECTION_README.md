...
The notebook under `research/legacy_notebooks/desktop_capture.ipynb` is for experimentation only.
...
## 8. Verify TensorFlow and DeepFace
...
If DeepFace reports a TensorFlow compatibility problem, reinstall the pinned
project requirements before debugging the camera.
...
The database settings belong only on the server running the API:
...
TensorFlow/DeepFace compatibility
invalid image
```
