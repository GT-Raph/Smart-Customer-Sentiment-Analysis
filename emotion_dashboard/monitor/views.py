...
def get_user_pc_prefix(user):
    """Return a local user's active branch prefix or deny unscoped access."""
    if user.is_superuser:
        return None
    if not user.branch_id:
        raise PermissionDenied("Your account has no branch assignment.")
    if not user.branch.is_active:
        raise PermissionDenied("Your branch assignment is inactive.")
...
def _scoped_query(request):
    get_user_pc_prefix(request.user)
...
def snapshot_image(
    ...
    """
    Protect captured images using the same branch restrictions as dashboard data.
    """
    snapshot = (
        visible_snapshots(
            request.user
    ...
    stored_path = Path(snapshot.image_path)
    image_path = (
        stored_path
